...
class RegisterChild(QDialog):
    ...
    def register_2(self):
        ...
        if self.child_nam == '':
            ...
            if self.birth_date.year() > QDate.currentDate().year()-13 or self.birth_date.year() < QDate.currentDate().year()-2:
                self.errors.setText("Please the child you registered must \nbe between the ages 2-12")
            else:
                if self.parent_nam == '':
                    ...
                    if self.emails == '':
                        ...
                        if self.phone_no == '':
                            ...
                            if age <= 5 and age > 2:
                                ...
                                col = 'Elementary'
                            elif age <= 12 and age > 9:
                                col = 'Preteens'
                            data = {"name":self.child_nam, "password":self.passcode, "phone":self.phone_no, "email":self.emails, "teacher":self.new_name, 'classe':col, 'birth_date':f'{self.birth_date.year()}/{self.birth_date.month()}/{self.birth_date.day()}', 'acc_type':'CE Account'}
